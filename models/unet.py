...
class DownBlock(nn.Module):
    ...
    def forward(
        ...
        x = self.residual_block(x, time_embedding)
        x = self.attention(x)
...
class TimeConditionedUNet(nn.Module):
    ...
    def forward(
        ...
        x, skip1 = self.down1(x, time_embedding)
        x, skip2 = self.down2(x, time_embedding)
        x, skip3 = self.down3(x, time_embedding)

        x = self.middle1(x, time_embedding)
        x = self.middle_attention(x)
        x = self.middle2(x, time_embedding)

        x = self.up1(x, skip3, time_embedding)
        x = self.up2(x, skip2, time_embedding)
        x = self.up3(x, skip1, time_embedding)
